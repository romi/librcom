...
namespace rcom {
        ...
        WebSocketFactory::WebSocketFactory(ILog& log, ISystem& system)
                : log_(log),
                  system_(system)
        {
        }
        ...
        WebSocketFactory::new_server_side_websocket(int sockfd)
        {
                Request request;
                RequestParser parser(request);
                std::unique_ptr<ISocket> socket
                        = std::make_unique<Socket>(log_, system_, sockfd);
                return std::make_unique<ServerSideWebSocket>(socket, parser, log_, system_);
        }
        
        std::unique_ptr<IWebSocket>
        WebSocketFactory::new_client_side_websocket(IAddress& remote_address)
        {
                Response response;
                ResponseParser parser(response);
                std::unique_ptr<ISocket> socket
                        = std::make_unique<Socket>(log_, system_, remote_address);
